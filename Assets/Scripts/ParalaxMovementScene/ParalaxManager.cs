...
public class ParalaxManager : MonoBehaviour {
    ...
    void Awake() {
        ...
        player.OnPlayerMoveX += OnPlayerMoveX;
    }

    void OnPlayerMoveX(float movement) {
        if(movement != cachedMovement) {
            SetScrollSpeed(movement);
            cachedMovement = movement;
        }
    }

    void SetScrollSpeed(float movement) {
        foreach(var background in backgrounds) {
